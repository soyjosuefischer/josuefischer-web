...
const CartItem = ({ item }) => {
  // Obtenemos las funciones del contexto del carrito
  const {removeFromCart, increaseAmount, decreaseAmount} = useContext(CartContext);
  // Extraemos los datos del producto del ítem
      ...
      <div className='w-full min-h-[150px] flex items-center gap-x-4'>
        ...
        <div className='w-full flex flex-col'>
          <div className='flex justify-between mb-2'>
            <Link to={`/product/${id}`} className='texts-sm uppercase font-medium max-w-[240px] text-primary hover:underline'>{title}</Link>
